								...
								<div class="box box-success" >
									<div class="box-body chat" id="chat-box">
										<div class="item" style="margin-bottom: -10px">
											<img src="<?= base_url('avatar/'.$publicacao->avatar)  ?>" alt="user image" class="online">
	
											<p class="message">
												<a href="#" class="name"> <?= $publicacao->nome  ?> </a>
												<a href="#" class="name"> <small class="text-muted pull-left"><i class="fa fa-clock-o"></i> <?= data_sql_para_dia_e_mes($publicacao->data) ?> as <?= $publicacao->hora ?></small> </a>
												<br>
											</p>
											<hr style="margin-top: 0px; margin-bottom: 0px">
											<h3><?= $publicacao->titulo ?></h3>
											<a href="<?= $publicacao->titulo ?>" style="font-style: italic;"><?= $publicacao->link ?></a>
											<p><?= $publicacao->descricao ?></p>
										</div>
									</div>
									<div class="box-footer clearfix">
										<?php if(in_array($publicacao->id_publicacao,$gostei)):  ?>
											<a href="#" class="btn btn-primary" id="gost_<?= $publicacao->id_publicacao  ?>" onclick="gostar_deixardegostar(<?= $this->session->userdata('id_usuario')?>,<?= $publicacao->id_publicacao?>)"><i class="fa fa-thumbs-o-up"></i> <span>Gostei</span></a>
										<?php else:  ?>
											<a href="#" class="btn btn-default" id="gost_<?= $publicacao->id_publicacao  ?>" onclick="gostar_deixardegostar(<?= $this->session->userdata('id_usuario')?>,<?= $publicacao->id_publicacao?>)" style="color: #3c8dbc;border-color: #367fa9;"><i class="fa fa-thumbs-o-up"></i> <span>Gostei</span></a>
										<?php endif;  ?>
									</div>
								</div>
						</section>
											...
											<ul class="treeview-menu">
												<?php foreach ($seguidos_list as $item) : ?>
													<li style="margin-bottom: 5px">
														...
														<!--span id="seg_m_<?= $publicacao->id_usuario  ?>" onclick="seguir_deixardeseguir(<?= $this->session->userdata('id_usuario')?>,<?= $publicacao->id_usuario?>)" class="btn btn-primary btn-xs pull-right"><i class="fa fa-check"></i></span-->
													</li>
												<?php endforeach;  ?>
											</ul>
										</li>
